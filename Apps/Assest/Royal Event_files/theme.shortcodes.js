	...
	jQuery(slider).each(function() {
		...
		if (last_width==0 || last_width!=width) {
			...
			if (ROYAL_EVENT_STORAGE['swipers'][id].params.slidesPerView != spv) {
				...
				//ROYAL_EVENT_STORAGE['swipers'][id].reInit();
			}
			...
			setTimeout(function() {
				...
				if ( ROYAL_EVENT_STORAGE['menuitems_list'].length > 1 ) {
					...
					jQuery('.popup_menuitem .sc_menuitems_wrap .sc_menuitem_content').append(
						  "<a class='prev_menuitem prevnext_menuitem show_popup_menuitem' data-href='"+ROYAL_EVENT_STORAGE['menuitems_list'][prev]+"'  href='#'><span class='icon-left'></span></a>"
